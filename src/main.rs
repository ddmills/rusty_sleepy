...
fn handle_input(state: &mut State, ctx: &mut BTerm) {
    state.map_view_mode = match ctx.key {
        None => state.map_view_mode,
        Some(key) => match key {
            VirtualKeyCode::T => MapViewMode::Temperature,
            VirtualKeyCode::E => MapViewMode::Elevation,
            VirtualKeyCode::P => MapViewMode::Precipitation,
            VirtualKeyCode::B => MapViewMode::Biome,
            VirtualKeyCode::R => {
                state.seed += 1;
                state.map.generate(state.seed);
                state.map_view_mode
            },
...
impl GameState for State {
    fn tick(&mut self, ctx: &mut BTerm) {
        ...
        for (i, _tile) in self.map.tiles.iter().enumerate() {
            let bg = match self.map_view_mode {
                MapViewMode::Biome => {
                    ...
                    if self.map.elevation[i] > SEA_LEVEL {
                        let t = self.map.temperature[i];
                        let p = self.map.precipitation[i];
                        c = self.biome_lut.get_hsv(t, p);
                    }
                    c.v = (c.v + self.map.elevation[i]) / 2.0;

                    c
                },
                MapViewMode::Elevation => HSV::from_f32(0.333, 0.5, self.map.elevation[i]),
                MapViewMode::Precipitation => HSV::from_f32(0.601, 0.5, 1.0 - self.map.precipitation[i]),
        ...
        draw.print(Point::new(1, 1), "(B) Biome (P) Precipitation (E) Elevation (T) Temperature");
